...
class expandStructAssign () =
  ...
  let rec expand lv ty loc =
    begin
    Printf.printf "expand logic var at (%d,%d)\n"
      (fst loc).Lexing.pos_lnum (fst loc).Lexing.pos_cnum;
    match unrollType ty with
      | TComp(mcomp,_,_) ->
          let field fi =
            let newlv = addOffsetLval (Field(fi,NoOffset)) lv in
	  ...
	  begin
          assert (not (is_reference_type ty));
          all_elem [] (array_size ty - 1L)
          (* Warning: old code
	     all_elem [] (direct_array_size ty - 1L) 
	     has problems with arrays of size to be computed at compile time
	  *)
	  end
...
class retypeAddressTaken =
  ...
  let postaction_expr e = match e.enode with
    | AddrOf(Var _v,NoOffset) ->
        e
        (* TODO unsupported "cannot take address of a function" *)
