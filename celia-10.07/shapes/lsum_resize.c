		     ...
		     ap_dimchange_t * dimchange, bool project)
{
  ...
  lsum_t *r = lsum_alloc_internal (pr, a->datadim, a->segmdim + 1);
  ap_dimchange_t dimadd;
  dimadd.realdim = 0;
#ifdef LSUM_REL
  dimadd.intdim = 3;
  dimadd.dim = (ap_dim_t *) malloc (3 * sizeof (ap_dim_t));
  dimadd.dim[0] = a->datadim + a->segmdim;
  dimadd.dim[1] = a->datadim + 2 * a->segmdim;
  dimadd.dim[2] = a->datadim + 3 * a->segmdim;
  r->dcons =
  ...
  dimadd.dim[1] = DATA_DIM (a->datadim, 0) + 2 * a->segmdim;
  r->dcons =
    ap_abstract0_add_dimensions (pr->man_dcons, false, a->dcons, &dimadd,
				 project);
  free (dimadd.dim);
			...
			ap_dimchange_t * dimchange)
{
  ...
  lsum_t *r =
    lsum_alloc_internal (pr, a->datadim, a->segmdim - dimchange->realdim);
  size_t i;
  ap_dimchange_t dimrm;
  dimrm.realdim = 0;
#ifdef LSUM_REL
  dimrm.intdim = 3 * dimchange->realdim;
  dimrm.dim = (ap_dim_t *) malloc (dimrm.intdim * sizeof (ap_dim_t));
  for (i = 0; i < dimchange->realdim; i++)
    {
      dimrm.dim[3 * i + 0] = dimchange->dim[i];
      dimrm.dim[3 * i + 1] = a->segmdim + dimchange->dim[i];
      dimrm.dim[3 * i + 2] = 2 * a->segmdim + dimchange->dim[i];
    }
...
#endif
#ifndef NDEBUG
  fprintf (stdout, "++ lsum_split - before substitute S(n1)\n");
  lsum_fdump (stdout, pr->man, r);
  fflush (stdout);
#endif
// S(n1) == S(n2) + d[n2]
  ...
  r->dcons =
    ap_abstract0_assign_linexpr (pr->man_dcons, true, r->dcons, sn1, expr,
				 NULL);
#ifndef NDEBUG
  fprintf (stdout, "++ lsum_split - after assign S(n1)\n");
  lsum_fdump (stdout, pr->man, r);
  fflush (stdout);
#endif
#ifdef LSUM_REL
// l[n1] == l[n2] + 1;
...
#ifndef NDEBUG
  fprintf (stdout, "++ lsum_split - before substitute l[n1]\n");
  lsum_fdump (stdout, pr->man, r);
  fflush (stdout);
#endif
  // l[n1] == l[n2] + 1;
  ...
  r->lcons =
    ap_abstract0_assign_linexpr (pr->man_lcons, true, r->lcons, ln1, expr,
				 NULL);
#ifndef NDEBUG
  fprintf (stdout, "++ lsum_split - after assign l[n1]\n");
  lsum_fdump (stdout, pr->man, r);
  fflush (stdout);
#endif
  // meet l[n2]-1 >= 0
