 ...
 * ushape_predicate.c
 *
 * Predicates on constrained heap graphs, extraction functions
 *
...
ushape_sat_lincons (ap_manager_t * man, ushape_t * a, ap_lincons0_t * lincons)
{
  ...
  switch (pcons->type)
	...
	if (sat && pr->size_scons > 0)
	  {
	    ...
	    ap_lincons0_t nlxy =
	      shape_lincons_of_node (pr, &lxy, v2n, a->h->size,
				     a->datadim, a->ptrdim);
	    free (v2n);
	    sat = false;
	...
	ap_lincons0_t ncons = pcons->info.data;
	sat = false;
	for (i = 0; i < pr->size_scons && !sat; i++)
	  sat =
...
ushape_sat_tcons (ap_manager_t * man, ushape_t * a, ap_tcons0_t * cons)
{
  ...
  switch (pcons->type)
	...
	size_t *v2n = hgraph_get_var2node (a->h);
	ap_lincons0_t ncons =
	  shape_lincons_of_node (pr, &(pcons->info.data), v2n,
