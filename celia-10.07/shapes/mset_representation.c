/*
 * mset_representation.c
 *
 * Functions related to the representation of multiset constraints.
 *
...
#else
  r->dcons =
    ap_abstract0_top (pr->man_dcons, DATA_DIM (intdim, 0) + 2 * r->segmdim,
		      0);
  r->lcons =
    ap_abstract0_top (pr->man_lcons, LEN_DIM (intdim, 0) + r->segmdim, 0);
#endif
  r->mscons =
...
mset_free_internal (mset_internal_t * pr, mset_t * a)
{
  if (a)
      ...
      if (a->dcons)
	{
      ...
      if (a->lcons)
	{
	  ap_abstract0_free (pr->man_lcons, a->lcons);
	  a->lcons = NULL;
	}
#endif
      if (a->mscons)
...
#ifndef MSET_REL
  s += ap_abstract0_size (pr->man_lcons, a->lcons);
#endif
...
#ifndef MSET_REL
  ap_abstract0_minimize (pr->man_lcons, a->lcons);
#endif
...
#ifndef MSET_REL
  ap_abstract0_canonicalize (pr->man_lcons, a->lcons);
#endif
...
#ifndef MSET_REL
  r->lcons = ap_abstract0_of_box (pr->man_dcons, LEN_DIM (intdim, 0), 0, t);
  r->dcons = ap_abstract0_of_box (pr->man_dcons, DATA_DIM (intdim, 0), 0, t);	/* TODO */
#else
...
{
#ifndef MSET_REL
//  ap_abstract0_manager_free(pr->man_lcons);
  pr->man_lcons = NULL;
#endif
...
#else
  pr->man_lcons = pk_manager_alloc (false);
  pr->man_dcons = pk_manager_alloc (false);
  pr->man_mscons = pkeq_manager_alloc ();
  man = ap_manager_alloc ("mset", "0.1 with (lcons=pk,dcons=pk,mscons=pkeq)", pr,
			  (void (*)(void *)) mset_internal_free);
#endif
