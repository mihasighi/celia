/*
 * hgraph_representation.c
 *
 * Functions related with heap graphs.
 *
...
/*
 * We consider adjacency matrices with nodes labeled by variables and some
 * additional symbols.
 *
...
/*
 * all program ptr variables are mapped to node n, other ptr vars are  mapped
 * to 0
 */
inline void
	       ...
	       node_t n)
{
  ...
  for (i = PTRVAR_NULL (pr, 0, ptrdim) + 1; i < ptrdim; i++)
    info[i].s = n;
}
...
/*
 * Returns the hgraph  with 1 node and dim ptr vars init to null!
 */
hgraph_t *
hgraph_alloc_null (hgraph_internal_t * pr, size_t datadim, size_t ptrdim)
{
  ...
  memset (r->info, 0, (ptrdim + 1) * sizeof (node_info_t));	/* ptr vars point to null
								 * node! */
  r->closed = true;
  return r;
}
...
/*
 * Returns the graph with size nodes (NOT IN THE HTABLE, NOT CANONICAL) and
		       ...
		       size_t datadim, size_t ptrdim)
{
  ...
  node_init_v2n (pr, r->info, ptrdim, NODE_T_TOP);	/* prg ptr vars point anywhere */
  // node_info_init (pr, r->info, ptrdim, size);
  return r;
}

/* Build graphs with two variables, dimensions are given */
hgraph_t *
	     ...
	     size_t realdim)
{
  ...
  hgraph_t *a;
  size_t x = PTRVAR_NULL (pr, datadim, ptrdim) + 1;
  size_t y = PTRVAR_NULL (pr, datadim, ptrdim) + 3;
  size_t z = PTRVAR_NULL (pr, datadim, ptrdim) + 5;
  size_t i;
  switch (code)
    {
    case 0:			/* x--> null */
	...
	VAR2NODE (a, y) = 2;
	/* set other vars to null */
	for (i = PTRVAR_NULL (pr, datadim, ptrdim) + 1; i < ptrdim; i++)
	  if (i != x && i != y)
	    VAR2NODE (a, i) = 0;
	break;
      }
    ...
    case 3:			/* x--> null and y --> _null and z --> null */
      {
	       ...
	       size_t realdim)
{
  ...
  /*
   * select radomly the number of cut nodes, encoded ptr vars and null is
   * already fixed by alloc_internal to 0, program ptr vars are fixed by
   * alloc_internal to size!
   */
  for (j = PTRVAR_NULL (pr, datadim, ptrdim) + 1, i = 1, ncut = 1; j < ptrdim;
       j++)
    {
      VAR2NODE (a, j) = 0;	/* put all prg ptr vars to null */
 ...
 *  for heap graphs:
 *  - each node is labeled by the min dimension
...
hgraph_check (hgraph_internal_t * pr, hgraph_t * a)
{
  ...
  /* encoded ptr vars allways point to 0 */
  for (i = 0; i <= PTRVAR_NULL (pr, a->datadim, a->ptrdim); i++)
    if (VAR2NODE (a, i) != 0)
      {
	ERROR ("invalid graph: ptr vars",;
	  );
	fprintf (stderr, "ptr var %zu points to %zu instead of 0\n",
		 (a->datadim + i), VAR2NODE (a, i));
	return '!';
      }
  /*
  ...
  /*
   * each node labeled by the min dimension or ptrdim * (nb of next) + min
   * dim
   */
  for (i = 0; i < a->size; i++)
    {
      if (NODE_VAR_NEXT (a, i) == 0)
	{
	  /* labeled cut node, verify that vi is the min dim for i */
	  size_t j;
      ...
      else if (NODE_VAR_NEXT (a, i) > 0 && NODE_VAR (a, i) == 0)
	{
...
/* Get the same content with size size; NULL positions may be added. */
int
...
/* Add a to dst by avoiding multiples copies. dst->p is changed. */
...
/*
 * Merge both arrays (which may be NULL). @return an array built from both or
 * NULL
 */
hgraph_array_t *
...
/* Return the sucessor in a of node n */
inline node_t
...
hgraph_node_get_succ_cut (hgraph_t * a, node_t n, int *anon)
{
  ...
  s = NODE_NEXT (a, n);
  *anon = 0;
  /* Loop finishes if not lasso */
  while (hgraph_node_is_cut (a, s) == false)
    {
      s = NODE_NEXT (a, s);
      *anon = *anon + 1;
    }
...
/* Return the nsucc sucessor on n in a */
size_t
...
{
  if (n < a->size && NODE_VAR (a, n) < a->ptrdim && NODE_VAR_NEXT (a, n) == 0)
    return true;
  return false;
}
...
{
  // This criteion is not good when folding is used since nodes for
  // folding shall be ordered
  // if (a->nn < b->nn || (a->nn == b->nn && a->v < b->v))
  if (a->v < b->v || (a->v == b->v && a->nn < b->nn))
    return true;
  else
    return false;
}
 ...
 * Sorting of nodes is done wrt lexicographical order on (nn,v).
...
hgraph_node_sort (hgraph_t * a, size_t i, ap_dimperm_t * perm)
{
  size_t j;
#ifndef NDEBUG
  fprintf (stdout, "\n hgraph_sort: from %zu on 'last with perm ", i);
  ...
  for (j = i + 1; j < a->size; j++)
	...
	// but garbage already marked with TOP
...
hgraph_node_set_var (hgraph_t * a, node_t n, size_t v, ap_dimperm_t * perm)
{
  ...
  /* case (2): infos changes but not need to move nodes */
  if (!sorted)
    {
 ...
 * Requires: the current sucessor of n is not defined
...
/* TODO: priority 3 */
/* Eliminate anonymous nodes after some limit */
void
