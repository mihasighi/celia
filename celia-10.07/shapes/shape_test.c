...
size_t realdim = 6+3; /* vars x,xi,y; add 3 for yi,z,zi */
size_t intdim = 3;  /* vars _l, _k, S; add 1 for m */
size_t SIZE = 4;     /* nb of nodes */

char* names_dim[15] = { "_l", "_k", "d", "_data", "_free", "_len",
                        "_next", "_new", "_null", "x", "xi", "y", "yi", "z", "zi" };
#define DIM_NULL (intdim+var_ptr_null)
#define DIM_X (intdim+var_ptr_null+1)
#define DIM_Y (intdim+var_ptr_null+3)
...
void test_misc(void)
{
  ...
  if (shape_is_dimension_unconstrained(ms,bot,var_ptr_null+1))
    printf("shape_is_dimension_unconstrained #12\n");
  if (!shape_is_dimension_unconstrained(ms,top,var_ptr_null+1))
...
test_add_pcons_once(exprmode_t mode, shape_t* o, pcons0_array_t* array)
{
    ...
    shape_pcons_array_fdump(stdout,array,0,0);
#endif
...
test_add_lincons(exprmode_t mode)
{
  ...
  ar.p[0] = shape_lincons_x_y_l(AP_CONS_DISEQ,1,DIM_Y,-1,DIM_NULL,0,0,intdim,realdim);
#ifdef PCONS
  arr = shape_pcons_array_of_lincons_array (pr, &ar, intdim, realdim);
  or = test_add_pcons_once(mode,o,arr);
#else
...
void test_assign(int subst, exprmode_t mode)
{
  ...
  /* shape 1 and xi := _null */
  o = shape_make(pr, 1, intdim,realdim);
  d = DIM_X+1;
  l = shape_linexpr_x_y_l(1,DIM_NULL,0,0,0,0,intdim,realdim);
  ...
  /* shape 1 and xi := x */
  l = shape_linexpr_x_y_l(1,DIM_X,0,0,0,0,intdim,realdim);
  or = shape_assign_linexpr_array(ms,false,o,&d,&l,1,NULL);
  FLAG(ms);
  RESULT(shape_check(pr,or));
...
void test_assign_texpr(int subst, exprmode_t mode)
{
  ...
  /* predefined graphs and constraints */
  /* case (1): code 1 and x := _null */
  /* shape 1 and xi := _null */
  o = shape_make(pr, 1, intdim,realdim);
  d = DIM_X+1;
  t1 = ap_texpr0_dim(DIM_NULL);
  ...
  d = DIM_Y;
  t4 = shape_texpr_x_next(DIM_X+1,1,intdim,realdim);
...
{
  shape_t *l7, *l9, *l10, *l11, *l12, *l13, *l14, *l15, *ljoin, *lwiden, *l16;
  ...
  xi = DIM_X+1; /* x_i */
  y = DIM_Y; /* y */
  t7 = ap_texpr0_dim(DIM_X); /* x */
  whileCond = ap_tcons0_array_make(1);
  whileCond.p[0] = shape_tcons_x_y (AP_CONS_DISEQ, xi, false, DIM_NULL, false, intdim, realdim);
  notWhileCond = ap_tcons0_array_make(1);
  notWhileCond.p[0] = shape_tcons_x_y (AP_CONS_EQ, xi, false, DIM_NULL, false, intdim, realdim);
  t10 = shape_texpr_x_y_v_cst_data(1,xi,0,0,0,0,2,true,intdim,realdim);
  t11 = shape_texpr_x_next(xi,1,intdim,realdim);
  t12 = ap_texpr0_dim(DIM_NULL); /* null */
  t13 = ap_texpr0_dim(y); /* y */
  t14 = t12; /* null */
  /*
   * init
   */
  l7 = shape_make(pr,1,intdim,realdim);
  l9 = shape_assign_texpr_array(ms,true,l7,&xi,&t7,1,NULL);
  for (i=0; i <= 4 && !fin; i++)
    {
...
void test_prg_add2copy(int code)
{
  ...
  z1 = DIM_Y+2; /* z1 */
  z2 = DIM_Y+3; /* z2 */
  t9 = ap_texpr0_dim(DIM_X); /* x */
  t10 = ap_texpr0_dim(DIM_Y); /* y */
  if (code <=1)
    {
      whileCond = ap_tcons0_array_make(1);
      whileCond.p[0] = shape_tcons_x_y (AP_CONS_DISEQ, xi, false, DIM_NULL, false, intdim, realdim);
  ...
  else
    {
      ...
      whileCond.p[0] = shape_tcons_x_y (AP_CONS_DISEQ, xi, false, DIM_NULL, false, intdim, realdim);
      whileCond.p[1] = shape_tcons_x_y (AP_CONS_DISEQ, yi, false, DIM_NULL, false, intdim, realdim);
    }
  notWhileCond = ap_tcons0_array_make(1);
  notWhileCond.p[0] = shape_tcons_x_y (AP_CONS_EQ, xi, false, DIM_NULL, false, intdim, realdim);
  t12 = shape_texpr_x_y_v_cst_data(1,xi,0,0,0,0,2,true,intdim,realdim);
  t13 = shape_texpr_x_next(xi,1,intdim,realdim);
  t14 = shape_texpr_x_next(yi,1,intdim,realdim);
  t15 = ap_texpr0_dim(DIM_NULL); /* null */
  t16 = ap_texpr0_dim(z1); /* z1 */
  t16b = ap_texpr0_dim(z2); /* z2 */
  /*
   * init
   */
  if (!code)
    ...
    l7 = shape_make(pr,4,intdim,realdim);
...
void test_prg_add2new(void)
{
  ...
  z = DIM_Y+2;
  n = DIM_NULL-2; /* new */
  t11 = ap_texpr0_dim(DIM_X); /* x */
  whileCond = ap_tcons0_array_make(1);
  whileCond.p[0] = shape_tcons_x_y (AP_CONS_DISEQ, xi, false, DIM_NULL, false, intdim, realdim);
  notWhileCond = ap_tcons0_array_make(1);
  notWhileCond.p[0] = shape_tcons_x_y (AP_CONS_EQ, xi, false, DIM_NULL, false, intdim, realdim);
  ifCond = ap_tcons0_array_make(1);
  ifCond.p[0] = shape_tcons_x_y (AP_CONS_EQ, yi, false, DIM_NULL, false, intdim, realdim);
  notIfCond = ap_tcons0_array_make(1);
  notIfCond.p[0] = shape_tcons_x_y (AP_CONS_DISEQ, yi, false, DIM_NULL, false, intdim, realdim);
  t13 = ap_texpr0_dim(n); /* new */
  t14 = shape_texpr_x_y_v_cst_data(1,xi,0,0,0,0,2,true,intdim,realdim);
  t15 = shape_texpr_x_next(DIM_NULL,-1,intdim,realdim);
  t17 = ap_texpr0_dim(z); /* z */
  t20 = shape_texpr_x_next(z,-1,intdim,realdim);
  t22 = ap_texpr0_dim(DIM_NULL); /* null */
  t25 = shape_texpr_x_next(xi,1,intdim,realdim);
  /*
   * init
   */
  l7 = shape_make(pr,1,intdim,realdim);
...
void test_prg_fstNot0(void)
{
  ...
  whileCond = ap_tcons0_array_make(2);
  whileCond.p[0] = shape_tcons_x_y (AP_CONS_DISEQ, xi, false, DIM_NULL, false, intdim, realdim);
  whileCond.p[1] = shape_tcons_x_y (AP_CONS_EQ, y, false, DIM_NULL, false, intdim, realdim);
  ifCond = ap_tcons0_array_make(1);
  ifCond.p[0] = ap_tcons0_make (AP_CONS_DISEQ,
      shape_texpr_x_y_v_cst_data (1,xi,0,0,0,0,0,false,intdim,realdim),NULL);
  notWhileCond = ap_tcons0_array_make(1);
  notWhileCond.p[0] = shape_tcons_x_y (AP_CONS_DISEQ, y, false, DIM_NULL, false, intdim, realdim);
  notIfCond = ap_tcons0_array_make(1);
  notIfCond.p[0] = ap_tcons0_make (AP_CONS_EQ,
      shape_texpr_x_y_v_cst_data (1,xi,0,0,0,0,0,false,intdim,realdim),NULL);
  t10 = ap_texpr0_dim(xi); /* xi */
  t12 = shape_texpr_x_next(xi,1,intdim,realdim); /* xi->next */
  t13 = ap_texpr0_dim(DIM_NULL); /* null */
  t14 = ap_texpr0_dim(z); /* z */
  /*
   * init
   */
...
void test_prg_getMax(void)
{
  ...
  t13 = shape_texpr_x_next(xi,1,intdim,realdim); /* xi->next */
  t14 = ap_texpr0_dim(DIM_NULL); /* null */
  t15 = ap_texpr0_dim(y); /* y */
  /*
   * init
   */
...
void test_prg_initMod2(void)
{
  ...
  *l15, *l16, *l17, *l18, *l19, *l20, *ljoin, *lwiden, *l21;
  ...
  t17 = shape_texpr_x_next(xi,1,intdim,realdim); /* xi->next */
  t18 = ap_texpr0_dim(DIM_NULL); /* null */
  t19 = ap_texpr0_dim(y); /* y */
  /*
   * init
   */
  l5 = shape_make(pr,1,intdim,realdim);
...
void test_prg_delAllGeV(void)
{
  ...
  *ljoin, *lwiden, *l31;
  ...
  t7 = ap_texpr0_dim(DIM_NULL); /* null */
  t8 = ap_texpr0_dim(x); /* x */
  whileCond = ap_tcons0_array_make(1);
  whileCond.p[0] = shape_tcons_x_y (AP_CONS_DISEQ, xi, false, DIM_NULL, false, intdim, realdim);
  ...
  ifCond2.p[0] = shape_tcons_x_y (AP_CONS_EQ, y, false, DIM_NULL, false, intdim, realdim);
  notWhileCond = ap_tcons0_array_make(1);
  notWhileCond.p[0] = shape_tcons_x_y (AP_CONS_EQ, xi, false, DIM_NULL, false, intdim, realdim);
  ...
  notIfCond2.p[0] = shape_tcons_x_y (AP_CONS_DISEQ, y, false, DIM_NULL, false, intdim, realdim);
  t11 = ap_texpr0_dim(xi); /* xi */
  t13 = shape_texpr_x_next(z,1,intdim,realdim); /* z->next */
  t18 = shape_texpr_x_next(DIM_NULL,-1,intdim,realdim); /* null / next */
  t19 = shape_texpr_x_next(xi,-1,intdim,realdim); /* xi / next */
  t21 = ap_texpr0_dim(intdim+var_ptr_free); /* free */
  t26 = shape_texpr_x_next(xi,1,intdim,realdim); /* xi->next */
  t28 = ap_texpr0_dim(z); /* z */
...
void test_prg_copyRev(void)
{
  ...
  *l17, *l18, *l19, *ljoin, *lwiden, *l20;
  ...
  notWhileCond = ap_tcons0_array_make(1);
  notWhileCond.p[0] = shape_tcons_x_y (AP_CONS_EQ, xi, false, DIM_NULL, false, intdim, realdim);
  t10 = ap_texpr0_dim(intdim+var_ptr_new); /* new() */
  t11 = shape_texpr_x_y_v_cst_data (1,xi,0,0,0,0,0,true,intdim,realdim); /* xi * data / _data */
  t12 = shape_texpr_x_next(y,-1,intdim,realdim); /* y / next */
  t14 = ap_texpr0_dim(z); /* z */
  t16 = shape_texpr_x_next(xi,1,intdim,realdim); /* xi->next */
