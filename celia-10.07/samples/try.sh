...
#		$MEMCHECK ./cinv-lsumprd$DEBUG.opt -debug 2 -domain shape $i &> log.txt
#		LOG=log/$FNAME-lsum-prd$NR
#		if test \( ! -d $LOG \)
#		then
#			mkdir $LOG
#		fi
...
#		$MEMCHECK ./cinv-lsumrel$DEBUG.opt -debug 2 -domain shape $i &> log.txt
#		LOG=log/$FNAME-lsum-rel$NR
#		if test \( ! -d $LOG \)
#		then
#			mkdir $LOG
#		fi
