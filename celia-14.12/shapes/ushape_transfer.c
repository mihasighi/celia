...
ushape_t *
ushape_assign_x_null(ushape_internal_t * pr, bool isfree, ushape_t * a, size_t vx);
...
ushape_assign_x_y(ushape_internal_t * pr, ushape_t * a, size_t vx, size_t vy) {
    ...
    ushape_t* anull = ushape_assign_x_null (pr, false, a, vx);
    hgraph_t *h = hgraph_assign_x_y(pr, anull->h, vx, vy, &perm);
    ushape_t *b = ushape_alloc_internal(pr, a->datadim, a->ptrdim);
    b->h = hgraph_copy_internal(pr, h);
    // TODO: free anull
    hgraph_free_internal(pr, h);
    ushape_apply_dimperm(pr, anull, b, &perm);
    ushape_free_internal(pr, anull);
    return b;
}
...
ushape_t *
ushape_assign_x_next_null(ushape_internal_t * pr, bool isfree, ushape_t * a, size_t vx);
...
ushape_assign_x_next_y(ushape_internal_t * pr, ushape_t * a, size_t vx,
    ...
    ap_dimperm_init(&perm, a->h->size);
    ap_dimperm_set_id(&perm);
    ushape_t *anull = ushape_assign_x_next_null(pr, false, a, vx);
    hgraph_t *h = hgraph_assign_x_next_y(pr, anull->h, vx, vy, &perm);
    ushape_t *b = ushape_alloc_internal(pr, a->datadim, a->ptrdim);
    b->h = hgraph_copy_internal(pr, h);
    hgraph_free_internal(pr, h);
    ushape_apply_dimperm(pr, anull, b, &perm);
...
ushape_assign_x_y_next(ushape_internal_t * pr, bool conservative,
    ...
    ushape_t* anull = ushape_assign_x_null (pr, false, a, vx);
    if (conservative)
        ap_dimperm_init(&perm, anull->h->size);
    else
        ap_dimperm_init(&perm, anull->h->size + 1);
    ap_dimperm_set_id(&perm);
    hgraph_t *h =
            hgraph_assign_x_y_next(pr, conservative, anull->h, vx, vy, &perm);
    ushape_t *b = ushape_alloc_internal(pr, a->datadim, a->ptrdim);
    b->h = hgraph_copy_internal(pr, h);
    hgraph_free_internal(pr, h);
    ushape_expand_internal(pr, anull, b, (ap_dim_t) VAR2NODE(a->h, vy),
            (conservative) ? 1 : 2);
...
ushape_assign_passign(ushape_internal_t * pr, ushape_t * a, passign0_t * op) {
    ...
    if (op->x < a->datadim || op->op == PA_ASSIGN_INT) { // data assignment, only a->scons affected
        ...
        if (op->offx == OFFSET_NONE &&
            ...
            /* put b as built in r */
        } else if (op->offx == OFFSET_NONE && op->op == PA_FREE) {
            // x := free
            b = ushape_assign_x_null(pr, true, a, vx);
            rsize += ushape_array_add(pr, true, r, 0, false, false, b);
        } else if (op->offx == OFFSET_NONE && op->op == PA_ASSIGN_PTR) {
            // x := [null | y->next]
            ...
            if (IS_PTRDIM(op->info.ptr.y, a->datadim, a->ptrdim) &&
                    ...
                    ny != 0 && op->info.ptr.offy == OFFSET_NONE) { // x := y
                ...
                rsize += ushape_array_add(pr, true, r, 0, false, false, b); /* put b as built in r */
                    ...
                    !IS_NULLDIM(op->info.ptr.y) &&
                    op->info.ptr.offy == ((int) (a->datadim + OFFSET_NEXT))) { // x := y->next
                // case (1): no new node
            ...
            // x->next = [null | y]
            if (op->offx != OFFSET_NONE && op->info.ptr.offy != OFFSET_NONE) {
