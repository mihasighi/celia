...
(** We compute the relation corresponding to the input heap. **)
  ...
  let apply
    ...
    let abs = tabs.(0) in
(*MS: to obtain abstraction to CA begin *
    let vpred = PSHGraph.predvertex graph hedge in
    let vout = open_out_gen [Open_creat;Open_wronly;Open_append] 0o666 "pan/trace.txt" in
    let fout = Format.formatter_of_out_channel vout in
    let _ = begin 
	Cinv_options.feedback 0 "write trace\n" ;
	Printf.fprintf vout "sid %d\n" (vpred.(0)).sid; 
        Format.fprintf fout "%a\n" print_point (vpred.(0));
        flush vout;
        close_out vout
    end in
 *MS: to obtain abstraction to CA end *)
