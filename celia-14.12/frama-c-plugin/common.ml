...
let fatal fmt = Celia_options.fatal ~current:true fmt

let notimplemented fmt =
  Celia_options.with_failure
    (fun evt -> raise (NotImplemented evt.Log.evt_message)) ~current:true fmt

let not_yet_implemented fmt =
  Celia_options.with_failure
    (fun evt -> raise (NotImplemented evt.Log.evt_message)) ~current:true fmt
...
let warning fmt = Celia_options.warning ~current:true fmt
let warn_general fmt = Celia_options.warning ~current:false fmt
...
let rec force_app_term_type f = function
  ...
  | Ltype _ | Lvar _ | Linteger | Lreal | Larrow _ as ty ->
      Celia_options.fatal "Unexpected non-C type %a" Printer.pp_logic_type ty
...
let get_unique_field ty = match unrollType ty with
  | TComp(compinfo,_,_) ->
	...
	| _ ->
	    Celia_options.fatal "Type %a has no unique field" Printer.pp_typ ty
      end
  | _ -> Celia_options.fatal "non-struct (unique field)"

let get_struct_name = function
  | TComp(compinfo,_,_) -> compinfo.cname
  | _ -> Celia_options.fatal "non-struct (name)"

let get_struct_info = function
  | TComp(compinfo,_,_) -> compinfo
  | _ -> Celia_options.fatal "non-struct (info)"
...
let integral_type_size_in_bytes ty =
  ...
  | TInt(IBool,_attr) -> (* TODO *)
      not_yet_implemented "Common.integral_type_size_in_bytes IBool"
  | TInt(ik,_attr) -> size_in_bytes ik
  (*MS: changed for Neon | TEnum _ -> theMachine.theMachine.sizeof_enum *)
  | TEnum _ -> Cil.bitsSizeOf ty / 8
  | _ -> assert false
...
let min_value_of_integral_type ?bitsize ty =
  ...
  match unrollType ty with
    ...
    | TEnum (einfo,_) ->
	(*MS: changed for Neon *)
	min_of (isSigned einfo.ekind) (size_in_bytes einfo.ekind)
    | _ -> assert false
...
let max_value_of_integral_type ?bitsize ty =
  ...
  match unrollType ty with
    ...
    | TEnum (einfo,_) ->
	(*MS: changed for Neon *)
	max_of (isSigned einfo.ekind) (size_in_bytes einfo.ekind)
    | _ -> assert false
...
let name_of_integral_type ?bitsize ty =
  ...
  match unrollType ty with
    ...
    | TEnum (einfo,_) ->
	(*MS: changed for Neon *)
	name_it (isSigned einfo.ekind) (size_in_bytes einfo.ekind)
    | _ -> assert false

(* Reference type *)

(* We introduce a reference type, that is different from the C pointer or
 * array type. It is a direct translation in C of the Clim bounded pointer
 ...
 * type. It will be translated in Clim in various alloc/free statements.
 *)
...
let reference_size ty =
  match findAttribute arraylen_attr_name (typeAttrs ty) with
    | [AInt i] -> Integer.to_int64 i (*MS changed for Neon Int64.of_int i *)
    | _ -> assert false
  ...
  let rec rec_size ?(top_size=false) ty =
    match unrollType ty with
	  ...
	  if is_reference_type ty && not top_size then
	    Int64.mul (rec_size (pointed_type ty)) (reference_size ty)
	  else
	  ...
	  if compinfo.cstruct then
	    match List.rev compinfo.cfields with
	      ...
	      | fi :: _ -> size_from_field fi
            ...
            (*MS no max on Int64 *)
	    List.fold_left (fun x y -> if Int64.compare x y > 0 then x else y)
                           Int64.zero (List.map size_from_field compinfo.cfields)
...
let is_bswap_function v = isFunctionType v.vtype && 
                          (String.contains v.vname '_') &&
                          (Str.string_partial_match (Str.regexp_string name_of_bswap)  v.vname 0)
...
object
  ...
  inherit Visitor.generic_frama_c_visitor
    ...
    (*MS solution 1, copy visitor (Cil.copy_visit (Project.current ())) as super *)
    (*MS solution 2, in place visitor *)
    (Cil.inplace_visit ()) as super 
  ...
  method vfunc f =
    ...
    match visitor#vfunc f with
	  ...
	  in
	  ChangeDoChildrenPost (f', postaction_func)
      | DoChildrenPost (f') -> DoChildrenPost (f')

  (* Inherit all other visitors *)

  (* Methods introduced by the Frama-C visitor *)
  method vfile = visitor#vfile
  (*MS removed for Neon method vrooted_code_annotation = visitor#vrooted_code_annotation *)
...
object
  ...
  (*MS removed for Neon method vrooted_code_annotation = visitor#vrooted_code_annotation *)
  method vglob_aux g =
    Celia_options.feedback "%a" Printer.pp_global g;
    visitor#vglob_aux g
  method vstmt_aux s =
    Celia_options.feedback "%a" Printer.pp_stmt s;
    visitor#vstmt_aux s

  (* Methods from Cil visitor for coding constructs *)
  method vfunc = visitor#vfunc
  method vblock b =
    Celia_options.feedback "%a" Printer.pp_block b;
    visitor#vblock b
  method vvrbl v =
    Celia_options.feedback "%s" v.vname;
    visitor#vvrbl v
  method vvdec v =
    Celia_options.feedback "%s" v.vname;
    visitor#vvdec v
  method vexpr e =
    Celia_options.feedback "%a" Printer.pp_exp e;
    visitor#vexpr e
  method vlval lv =
    Celia_options.feedback "%a" Printer.pp_lval lv;
    visitor#vlval lv
  method voffs off =
    Celia_options.feedback "%a" Printer.pp_offset off;
    visitor#voffs off
  method vinitoffs off =
    Celia_options.feedback "%a" Printer.pp_offset off;
    visitor#vinitoffs off
  method vinst i =
    Celia_options.feedback "%a" Printer.pp_instr i;
    visitor#vinst i
  method vinit = visitor#vinit
  method vtype ty =
    Celia_options.feedback "%a" Printer.pp_typ ty;
    visitor#vtype ty
  method vattr attr =
    Celia_options.feedback "%a" Printer.pp_attribute attr;
    visitor#vattr attr
  method vattrparam pattr =
    Celia_options.feedback "%a" Printer.pp_attrparam pattr;
    visitor#vattrparam pattr

  (* Methods from Cil visitor for logic constructs *)
  method vlogic_type lty =
    Celia_options.feedback "%a" Printer.pp_logic_type lty;
    visitor#vlogic_type lty
  method vterm t =
    Celia_options.feedback "%a" Printer.pp_term t;
    visitor#vterm t
  method vterm_node = visitor#vterm_node
  method vterm_lval tlv =
    Celia_options.feedback "%a" Printer.pp_term_lval tlv;
  ...
  method vlogic_var_decl lv =
    Celia_options.feedback "%a" Printer.pp_logic_var lv;
  ...
  method vpredicate_named p =
    Celia_options.feedback "%a" Printer.pp_predicate_named p;
...
*)
  ...
  method vspec funspec =
    Celia_options.feedback "%a" Printer.pp_funspec funspec;
  ...
  method vcode_annot annot =
    Celia_options.feedback "%a" Printer.pp_code_annotation annot;
    visitor#vcode_annot annot
  method vannotation annot =
    Celia_options.feedback "%a" Printer.pp_global_annotation annot;
...
(*MS: commented for Neon because code is not more compiling  
let do_on_term_offset (preaction_offset,postaction_offset) tlv =
  let preaction_toffset tlv =
    match preaction_offset with None -> tlv | Some preaction_offset ->
      let lv,env =
        !Db.Properties.Interp.term_offset_to_offset None tlv
      in
...
let print_to_stdout file =
  ...
  (* Log.print_on_output "%a" Printer.pp_file Format.std_formatter file *)
...
object
  ...
  inherit Visitor.generic_frama_c_visitor
    (*MS change for Neon (Project.current ()) (Cil.inplace_visit ()) as super *)
    (Cil.inplace_visit ()) as super
  ...
  method vterm t =
    match t.term_node with
	  ...
	  begin match t.term_type with
		...
		Celia_options.fatal ~current:true "Term %a has void type" Printer.pp_term t
	    | _ -> DoChildren
	  end
      | _ -> DoChildren
    ...
    | Field(fi,_) ->
        begin
          try
            ...
            then
	      Celia_options.warning ~current:true
...
let change_idx idx1 idx siz =
  ...
  *)
  let boff = Logic_const.term (TBinOp(Mult, idx1, Logic_const.tinteger (Integer.to_int siz))) (Cil_types.Linteger) in
  Logic_const.term (TBinOp(PlusA,boff,idx)) (Cil_types.Linteger) 
  ...
  match off with
      TIndex(idx1,(TIndex _ as suboff)) ->
        ...
        begin match lift_toffset subty suboff with
            ...
            | TField _ | TNoOffset | TModel _ -> assert false
        end
    | TIndex(idx1,TNoOffset) ->
        ...
        if isArrayType subty then
          let siz = array_size subty in
          (*MS changed for Neon  *)
          TIndex(change_idx idx1 (Logic_const.tinteger 0) siz, TNoOffset)
        else off
    | TIndex _ | TField _ | TNoOffset | TModel _ -> off
...
let mkalloc_statement v ty loc = mkStmt (Cil_types.Instr(mkalloc v ty loc))

let mkalloc_array v ty num loc =
  let callee = new_exp ~loc (Lval(Var(malloc_function ()),NoOffset)) in
  let arg = constant_expr (Integer.mul num (Integer.of_int (bytesSizeOf ty))) in
  Call(Some(Var v,NoOffset),callee,[arg],loc)

let mkalloc_array_statement v ty num loc =
  mkStmt (Cil_types.Instr(mkalloc_array v ty num loc))
...
let mkfree_statement v loc = mkStmt (Cil_types.Instr(mkfree v loc))

(* from cilutil.ml in version Carbon *)
(* Memoize *)
