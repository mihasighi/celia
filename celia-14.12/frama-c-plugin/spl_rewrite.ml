...
class rewrite_vars =
  ...
  inherit Visitor.generic_frama_c_visitor
    (*MS changed for Neon Project.current ()*) (Cil.inplace_visit ())

  method vglob_aux = function
    | GFun (fundec,_) ->
      begin
	if (String.compare fundec.svar.vname (FCKernel.MainFunction.get())) == 0 then
	  DoChildren
	else
	  SkipChildren
      end
  ...
  method vexpr e = 
    match e.enode with
	  ...
	  if !local_cast then 
	    ...
	    Printf.fprintf !spl_out "%s %s" (Integer.to_string v) (top_exp_stmt ())
	      ;
	  (pop_exp_stmt ());
	  SkipChildren
