  ...
  method vvdec v =
    if isArrayType v.vtype && not (Cil_datatype.Varinfo.Set.mem v !varset) then
      begin
        ...
        let newty =
          if array_size v.vtype > Integer.zero then
            begin
  ...
  method vlogic_var_decl lv =
    if not (Cil_datatype.Logic_var.Hashtbl.mem lvar_to_strawlvar lv) &&
      ...
      begin
        ...
        let newty =
          if force_app_term_type array_size lv.lv_type > Integer.zero then
            begin
              let size =
...
*)
	    ...
	    Cil_const.make_logic_info (unique_logic_name ("valid_" ^ v.vname)) in
          let trange = Common.mkterm 
			 (Trange(Some (constant_term v.vdecl Integer.zero),
				 Some (constant_term v.vdecl (Integer.sub size Integer.one))))
		         Linteger loc
	  in
          let p =
            Pvalid(
              LogicLabel(None, "Here"), 
              (Common.mkterm (TBinOp(PlusPI,
              		       variable_term v.vdecl (cvar_to_lvar v),
			       trange))
			    Linteger loc))
          in
          globinv.l_labels <- [ LogicLabel(None,"Here") ];
          globinv.l_body <- LBpred (mkpredicate []  v.vdecl p);
...
*)
		...
		in
                let p =
                  Pvalid(LogicLabel(None,"Here"), TBinOp(PlusPI,
                    variable_term v.vdecl (cvar_to_lvar v),
  		    Trange(
...
object(self)
  ...
  method private postaction_lval lv =
    match lv with
      ...
      | Var _, _ ->
          (*MS: TODO change to support arrays in logic *)
	unsupported "cannot handle v+offset lvalue"
...
let normalize file =
  ...
  Celia_options.debug "Retype variables of array type";
  retype_array_variables file;
  if checking then check_types file;
  (* Retype logic functions/predicates with structure parameters or return. *)
  Celia_options.debug "Retype logic functions/predicates";
  retype_logic_functions file;
  if checking then check_types file;
  (* Expand structure copying through parameter, return or assignment. *)
  (* order: before [retype_address_taken], before [retype_struct_variables] *)
  Celia_options.debug "Expand structure copying";
  expand_struct_assign file;
  if checking then check_types file;
  (* Retype variables of structure type. *)
  Celia_options.debug "Retype variables of structure type";
  retype_struct_variables file;
  if checking then check_types file;
  (* Retype variables and fields whose address is taken. *)
  (* order: after [retype_struct_variables] *)
  Celia_options.debug "Retype variables and fields whose address is taken";
  ...
  Celia_options.debug "Expand structure copying through assignment";
  expand_struct_assign file;
  if checking then check_types file;
  (* Translate union fields into structures. *)
  Celia_options.debug "Translate union fields into structures";
  translate_unions file;
  if checking then check_types file;
  (* Retype fields of type structure and array. *)
  (* order: after [expand_struct_assign] and [retype_address_taken]
   * before [translate_unions] *)
  Celia_options.debug "Retype fields of type structure and array";
  retype_fields file;
  if checking then check_types file;
  (* Retype fields of type structure and array. *)
  ...
  Celia_options.debug "Remove array address";
  remove_array_address file;
  if checking then check_types file;
  (* Retype type tags. *)
  (* order: before [retype_base_pointer] *)
  Celia_options.debug "Retype type tags";
  retype_type_tags file;
  if checking then check_types file;
  (* Retype pointers to base types. *)
  (* order: after [retype_fields] *)
  Celia_options.debug "Retype pointers to base types";
  retype_base_pointer file;
  if checking then check_types file;
  (* Remove useless casts. *)
  Celia_options.debug "Remove useless casts";
