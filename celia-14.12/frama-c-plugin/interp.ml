...
let ctype (* ?bitsize *) ty =
  let tnode = match unrollType ty with
    ...
    | TFloat _ -> unsupported "Datatype.float not allowed"
...
and lval pos = function
  ...
  | Mem _e, Index _ as lv ->
      Celia_options.fatal ~current:true "Unexpected lval %a" Printer.pp_lval lv
...
let rec statement s =
  let pos = Cil_datatype.Stmt.loc s.skind in
  CurrentLoc.set pos;
(*
  let assert_list =
...
let global vardefs g =
  let pos = Cil_datatype.Global.loc g in
  CurrentLoc.set pos;
    ...
    | GCompTag(compinfo,_) when compinfo.cstruct -> (* struct type *)
        ...
        let fields =
          List.fold_right (fun fi acc ->
                             ...
                             if Cil_datatype.Fieldinfo.equal fi repfi then
                               field fi @ acc
                             else acc
                          ) compinfo.cfields []
        in
