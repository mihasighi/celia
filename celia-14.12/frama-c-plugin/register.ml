  ...
  try
    ...
    *)
    ...
    begin
      ...
      let gd = Kernel.GeneralDebug.get () in
      let _ = Kernel.GeneralDebug.set (gd+1) in
      let out_chan = open_out "pan/pan-nm.c" in
      let fmt = Format.formatter_of_out_channel out_chan in
      (* let printer = new Printer.print () in 
      Format.fprintf fmt "%a" (Printer.pp_file printer) file; *)
      Printer.pp_file fmt file;
      flush out_chan;
      close_out out_chan;
      Kernel.GeneralDebug.set gd
    end; 
