...
class add_default_behavior =
  object(_self)
    inherit Visitor.generic_frama_c_visitor (*MS changed for Neon Project.current() *)
      (Cil.inplace_visit())
    method vspec s =
...
object
  ...
  inherit Visitor.generic_frama_c_visitor
    (*MS changed for Neon Project.current ()*) (Cil.inplace_visit ()) as super
...
(*MS: changed because does no more compile
  let pool = ref Cil_datatype.Global_annotation.Set.empty in
  Annotations.iter_global 
    (fun _ annot ->
	 ...
	 | Dtype_annot _ | Dinvariant _ 
         | Dvolatile _ | Dmodel_annot _ | Dcustom_annot _ ->
	     (* Useful ? harmless ?
...
*)
    ...
    in
    let pstring = (*MS for Neon: changed *)
      Logic_const.papp (validstring,[],[variable_term v.vdecl (cvar_to_lvar v)])
    in
    ...
    in
    let size = constant_term v.vdecl size (*MS changed for Neon Int64.of_int size *) in
    let psize = Logic_const.prel (Req,strsize,size) in
    (*MS for Neon changed predicate to Logic_const.unamed *)
    let p = Logic_const.pand ~loc:v.vdecl (pstring,psize) in
    let globinv =
  ...
  method vexpr e = match e.enode with
	...
	let v =
	  findOrAdd string_to_var s
	    (fun s ->
	       make_glob ~wstring:false (string_var s) (Integer.of_int (String.length s))
		 (string_cabs_init s))
	in
	...
	let v =
	  findOrAdd wstring_to_var ws
	    (fun ws ->
	       make_glob ~wstring:true (wstring_var ()) (Integer.of_int (List.length ws - 1))
...
let gather_initialization file =
  do_and_update_globals
	...
	let _s = match iinfo.init with
	  | Some ie ->
	      let b = Cabs2cil.blockInit false (Var v, NoOffset) ie v.vtype in
	      b.bstmts
	  | None ->
  ...
  let preaction_expr e = match e.enode with
	...
	new_exp ~loc:e.eloc
          (BinOp(op,
	         ...
	         constant_expr ~loc:e.eloc Integer.zero,ty))
    | _ -> e
  in
object
  ...
  | CastE(ty,e) ->
	...
	&& (* typeSig (typeRemoveAttributes ["const";"volatile"]
	    ...
	    typeSig (typeRemoveAttributes ["const";"volatile"]
            ...
            (type_remove_attributes_for_c_cast (unrollType (pointed_type ety)))
            =
            (type_remove_attributes_for_c_cast (unrollType (pointed_type ty)))
      then
        ...
        begin try match destruct_pointer e1,destruct_pointer e2 with
		  ...
		  try Some(expr_offset v2) with Not_found -> None in
		      ...
		      off1
                  ...
                  | None,None ->
		      constant_expr ~loc:e.eloc Integer.zero (* 0L *)
              else e
...
(*MS: comment to avoid introduction of these names. Any problem?
  method vfunc f =
    let local v =
	...
	if v.vformal then
	  ...
	  begin try
	    ...
	    let eoff = match destruct_pointer e with
	      ...
	      | Some(v2,None) ->
		  begin try expr_offset v2
		  with Not_found -> constant_expr ~loc:e.eloc Integer.zero end
	    in
	    ChangeDoChildrenPost
	      ([Set((Var voff,NoOffset),eoff,loc)], fun x -> x)
	  with Not_found -> DoChildren end
...
class annotateCodeStrlen(strlen : logic_info) =
  ...
  let lval_destruct_string_access ~through_tmp = function
	...
	begin match destruct_pointer e with
	  ...
	  | Some(v,None) -> Some(v,constant_expr ~loc:e.eloc Integer.zero)
	end
    | Var v, off ->
...
object(self)
  ...
  method vexpr e =
	...
	let off = Logic_utils.expr_to_term false off in
        (*MS changed for Neon !Db.Properties.Interp.force_exp_to_term off in *)
	let app = within_bounds ~strict:false v off in
	let cur_stmt = the self#current_stmt in
	(*MS changed for Neon Annotations.add_alarm *)
	Annotations.add_assert Emitter.kernel (*MS to see *)
	  cur_stmt
	  (* [ Celia_options.Analysis.self ]
	  ~before:true
	  Alarms.Other_alarm *)
	  app
    end;
  ...
  method vstmt_aux s =
	...
	(*MS: HintLevel not introduced yet! Make the test false *)
	(* if Celia_options.HintLevel.get () > 1 then *)
	if false then
		...
		let off = Logic_utils.expr_to_term false off in
		  ...
		  let lv' = Logic_utils.lval_to_term_lval false lv in
        	(*MS changed for Neon !Db.Properties.Interp.force_lval_to_term_lval lv in *)
		  let e' = Logic_utils.expr_to_term false e in
        	(*MS changed for Neon !Db.Properties.Interp.force_exp_to_term e in *)
		  ...
		  in
		  (*MS changed for Neon  Annotations.add_alarm *)
 		  Annotations.add_assert Emitter.kernel
		    s
		    (* [ Celia_options.Analysis.self ]
		    ~before:false
		    Alarms.Other_alarm *)
  ...
  method vexpr e = match e.enode with
	...
	if isSignedInteger ty1 then
	  begin match possible_value_of_integral_expr e2' with
	    | Some i when i >= Integer.zero -> ()
	    | _ ->
		let t2' = Logic_utils.expr_to_term false e2' in
                let tzero = Logic_const.tinteger ~loc:e.eloc 0 in
                  (*MS changed for Neon 
		       new_exp ~loc:e.eloc (BinOp(Ge,e2',constant_expr ~loc:e.eloc 0L,intType)) *)
		let rel =
		    Logic_const.new_predicate (Logic_const.prel (Rge,t2',tzero))
		  (*MS changed for Neon
	...
	begin match possible_value_of_integral_expr e2' with
	  ...
	  | _ ->
                  (*MS changed for Neon 
	      let max_right = constant_expr ~loc:e.eloc max_right in
	      let check =
		...
		!Db.Properties.Interp.force_exp_to_predicate check
	      in *)
	      let t2' = Logic_utils.expr_to_term false e2' in
              let tmax = Logic_const.tint ~loc:e.eloc max_right in
	      let rel = Logic_const.new_predicate (Logic_const.prel (Rlt,t2',tmax))
	      in
	      let check =
		    Logic_const.pred_of_id_pred
		      { rel with ip_name = [ name_of_hint_assertion ] }
	...
	(*MS changed for Neon
		let check =
                  new_exp ~loc:e.eloc
                    (BinOp(Ge,e1',constant_expr ~loc:e.eloc 0L,intType)) in
		let check =
		  !Db.Properties.Interp.force_exp_to_predicate check
		in *)
	      let t1' = Logic_utils.expr_to_term false e1' in
              let tzero = Logic_const.tinteger ~loc:e.eloc 0 in
	      let rel = Logic_const.new_predicate (Logic_const.prel (Rge,t1',tzero))
	      in
	      let check =
		    Logic_const.pred_of_id_pred
		      { rel with ip_name = [ name_of_hint_assertion ] }
	...
	if is_left_shift && isSignedInteger ty1 then
	  ...
	  in
	  begin match possible_value_of_integral_expr e2' with
	    | Some i when i >= Integer.zero && i < (Integer.of_int64 64L) ->
		(* Only use optimization where [Int64.shift_right] is
		 * defined in OCaml
		 *)
		(*MS changed for Neon let i = Int64.to_int i in *)
                let t1' = Logic_utils.expr_to_term false e1' in
		let tmax_left = Logic_utils.expr_to_term false (constant_expr ~loc:e.eloc (Integer.of_int64 (Int64.shift_right max_int (Integer.to_int i)))) in
	        let rel = Logic_const.new_predicate (Logic_const.prel (Rle,t1',tmax_left))
                (*MS changed for Neon  new_exp ~loc:e.eloc (BinOp(Le,e1',max_left,intType)) *)
	        in
	        let check =
		    Logic_const.pred_of_id_pred
		      { rel with ip_name = [ name_of_hint_assertion ] }
		...
		(*MS changed for Neon Annotations.add_alarm *)
		  ...
		  check
	    | _ ->
		let max_int = constant_expr ~loc:e.eloc (Integer.of_int64 max_int) in
		let tmax_left = Logic_utils.expr_to_term false 
			(new_exp ~loc:e.eloc (BinOp(Shiftrt,max_int,e2',intType))) in
                let t1' = Logic_utils.expr_to_term false e1' in
	        let rel = Logic_const.new_predicate (Logic_const.prel (Rle,t1',tmax_left))
                (*MS changed for Neon  
		let max_left =
...
let rewrite file =
   ...
   *)
  Celia_options.debug "Adding default behavior to all functions";
  add_default_behavior file;
  if checking then check_types file;
  (* Rename entities to avoid conflicts with Celia predefined names.
     Should be performed before any call to [Cil.cvar_to_lvar] destroys
     sharing among logic variables.
  *)
  Celia_options.debug "Rename entities";
  rename_entities file;
  if checking then check_types file;
  (* Fill offset/size information in fields *)
  Celia_options.debug "Fill offset/size information in fields";
  fill_offset_size_in_fields file;
  if checking then check_types file;
  (* Replace addrof array with startof. *)
  Celia_options.debug "Replace addrof array with startof";
  replace_addrof_array file;
  if checking then check_types file;
  (* Replace string constants by global variables. *)
  Celia_options.debug "Replace string constants by global variables";
  replace_string_constants file;
  if checking then check_types file;
  (* Put all global initializations in the [globinit] file. *)
  (* Replace global compound initializations by equivalent statements. *)
  Celia_options.debug "Put all global initializations in the [globinit] file";
  gather_initialization file;
  if checking then check_types file;
  (* Rewrite comparison of pointers into difference of pointers. *)
  (*MS: removed 
  if Celia_options.InferAnnot.get () <> "" then
    begin
      Celia_options.debug "Rewrite comparison of pointers into difference of pointers";
      rewrite_pointer_compare file;
      if checking then check_types file
    end;
  *)
  (* Rewrite type void* and (un)signed char* into char*. *)
  Celia_options.debug "Rewrite type void* and (un)signed char* into char*";
  ...
  if Celia_options.InferAnnot.get () <> "" then
    begin
      Celia_options.debug "Rewrite cursor pointers into offsets from base pointers";
  ...
  *)
  (* Rewrite cursor integers into offsets from base integers. *)
    ...
    (* if Celia_options.InferAnnot.get () <> "" then *)
    begin
      Celia_options.debug "Rewrite cursor integers into offsets from base integers";
      rewrite_cursor_integers file;
      if checking then check_types file
    end;
  (* Annotate code with strlen. *)
  (*MS: never done *)
  (* if Celia_options.HintLevel.get () > 0 then *)
  if false then
    begin
      Celia_options.debug "Annotate code with strlen";
      annotate_code_strlen file;
      if checking then check_types file
    end;
  (* Annotate code with overflow checks. *)
  Celia_options.debug "Annotate code with overflow checks";
