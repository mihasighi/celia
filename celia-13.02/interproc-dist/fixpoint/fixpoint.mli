...
(** The manager parameterizes the fixpoint solver with the
    ...
    {- Debugging options, for text output on the formatter [print_fmt];}
...
type ('vertex,'hedge,'abstract,'arc) manager =
  ...
  mutable widening_start : int;
	(** First widening step in a strategy *)
...
type ('vertex,'hedge) strategy = ('vertex,'hedge) strategy_vertex Ilist.t
	...
	there is an edge from [6] to [2], the loop will not be iterated.
      ...
      See {!make_strategy_default} to generate automatically a sound strategy.
    *)
