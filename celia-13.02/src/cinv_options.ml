...
(*  ---------------------------------------------------------------------- *)
(** {5 Choice of analysis direction} *)
(*  ---------------------------------------------------------------------- *)
type direction =
  | Forward
  | Backward
        ...
        Arg.String(begin fun str ->
          ...
          String.iter
            (begin fun chr ->
              ...
              | _ ->
                  raise (Arg.Bad ("Wrong argument `"^str^"'; option `-direction' expects only 'f' or 'b' characters in its argument string"))
            end)
            str;
          direction := List.rev !direction;
          if !direction=[] then direction := [Forward];
        end),
