  ...
  object(_self)
    ...
    method vspec s =
	  ...
	  (* { b_name = Cil.default_behavior_name;
	    b_assumes = []; (* must be always empty for default_behavior_name *)
	    b_requires = [];
	    b_assigns = WritesAny;
	    b_post_cond = [Normal, Logic_const.new_predicate Logic_const.ptrue];
	    b_extended = [];
	  } *) (* Cil.mk_behavior () *) (** @since Carbon-20101201 *)
	  (Cil.mk_behavior 
	     ~post_cond:([Normal, Logic_const.new_predicate Logic_const.ptrue])
	     ())
	:: s.spec_behavior
      end;
      SkipChildren
  ...
  let preaction_expr e = match e.enode with
	...
	new_exp ~loc:e.eloc
          (BinOp(op,
	         ...
	         constant_expr ~loc:e.eloc 0L,ty))
    | _ -> e
  in
object
	...
	if v.vformal then
	  ...
	  begin try
	    ...
	    let eoff = match destruct_pointer e with
	      ...
	      | Some(v2,None) ->
		  begin try expr_offset v2
		  with Not_found -> constant_expr ~loc:e.eloc 0L end
	    in
	    ChangeDoChildrenPost
	      ([Set((Var voff,NoOffset),eoff,loc)], fun x -> x)
	  with Not_found -> DoChildren end
...
class annotateCodeStrlen(strlen : logic_info) =
  ...
  let lval_destruct_string_access ~through_tmp = function
	...
	begin match destruct_pointer e with
	  ...
	  | Some(v,None) -> Some(v,constant_expr ~loc:e.eloc 0L)
	end
    | Var v, off ->
  ...
  method vexpr e = match e.enode with
	...
	if is_left_shift && isSignedInteger ty1 then
	  ...
	  begin match possible_value_of_integral_expr e2' with
		...
		let max_left = constant_expr ~loc:e.eloc (Int64.shift_right max_int i) in
		let check =
		...
		Annotations.add_alarm
		  ...
		  check
	    | _ ->
		let max_int = constant_expr ~loc:e.eloc max_int in
		let max_left =
