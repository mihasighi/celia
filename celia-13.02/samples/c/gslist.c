 ...
 * g_slist_append:
 * @list: a #GSList
 * @d: the data for the new element
 *
...
g_slist_insert(GSList *list,
    ...
    if (position < 0)
        return g_slist_append(list, d);
    else if (position == 0)
        return g_slist_prepend(list, d);
 ...
 * @slist: a #GSList
 * @sibling: node to insert @d before
 * @d: data to put in the newly-inserted node
 *
 * Inserts a node before @sibling containing @d.
 * 
 * Returns: the new head of the list.
 */

/**
 @ requires acyclic(sibiling) && reach(slist,sibiling);
 */
GSList*
g_slist_insert_before(GSList *slist,
    ...
    if (!slist) {
        ...
        GSList *node, *last = NULL, *tmp = NULL;
...
/**
 @ requires acyclic(list1) && acyclic(list2) && disjoint(list1,list2);
 */
GSList *
 ...
 * g_slist_remove:
 * @list: a #GSList
 * @d: the data of the element to remove
 *
 ...
 */
/**
 @ requires acyclic(link_) && reach(list,link_);
 */
GSList*
 ...
 */
/**
 @ requires acyclic(llink) && reach(list,llink);
 */
gint
...
g_slist_foreach(GSList *list,
        /* GFunc func, */ // MS: instantiate with + function
        gpointer user_data) {
    GSList *next;
    while (list) {
        next = list->next;
        ...
        gpointer user_data) */
{
    ...
    cmp = d - tmp_list->data;
    tmp_nxt = tmp_list->next;

    while ((tmp_nxt != NULL) && (cmp > 0)) {
        prev_list = NULL;
        prev_list = tmp_list;
        tmp_list = NULL;
        tmp_list = prev_list->next;
        tmp_nxt = NULL;
        tmp_nxt = tmp_list->next; // PB
    ...
    if (prev_list) {
        ...
        tmp_list = tmp_nxt = prev_list = NULL;
        return list;
    } else {
        new_list->next = NULL;
        new_list->next = list;
...
g_slist_insert_sorted(GSList *list,
        gpointer d) /*,
        GCompareFunc func) */ // MS: only difference
{
    return g_slist_insert_sorted_real(list, d);
}
...
g_slist_insert_sorted_with_data(GSList *list,
        ...
        gpointer user_data) */
{
    return g_slist_insert_sorted_real(list, d); /*, (GFunc) func, user_data); */
}
...
g_slist_sort_real(GSList *list) /*,
    ...
    // MS: splits list in two sublists by taking on over two elements
    ...
    // MS: Apply recursivelly the function
    /*
     ...
     */
     l1 = l1_next = NULL;
     l1_next = g_slist_sort_real(list);
     l2_next = NULL;
     l2_next = g_slist_sort_real(l2);
     l2 = NULL;
     l2 = g_slist_sort_merge(l1_next,l2_next);
     l1_next = l2_next = NULL;
     return l2;
}
