...
hgraph_assign_passign(hgraph_internal_t * pr, bool conservative,
    ...
    if (op->lhs < a->datadim || op->type == DATA_MODIFIER) { // data assignment, do nothing
    ...
    if (op->type == NO_MODIFIER) { // x := [null | new | free | y | y->next]
        if (IS_PTRDIM(op->info.ptr.y, a->datadim, a->ptrdim) &&
            ...
            if (conservative) { // case (1): no new node
                ap_dimperm_init(&perm, a->size);
                ap_dimperm_set_id(&perm);
        ...
        } else if (op->op == PA_ALLOC || op->op == PA_ALLOC_N) { // x := new
            if (!conservative) {
                ap_dimperm_init(&perm, a->size);
                ap_dimperm_set_id(&perm);
