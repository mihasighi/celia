    ...
    *rsize = 0;
    ...
    if (nnx != NODE_T_TOP && nny != NODE_T_TOP) {
        ...
        *rsize += ushape_array_add(pr, true, r, *rsize, true, true, b); /* copy and destroy */
        ...
        } else if (nnfixed != NODE_T_TOP) /* nextany == 0 */ { /* vfixed[->next] = vany */
            /* set vany to nnfixed */
            b = ushape_copy_set_var(pr, a, nnfixed, vany, 0, 0);
            *rsize += ushape_array_add(pr, true, r, *rsize, true, true, b); /* copy and destroy */
        } else { /* nextany == 0 && nnfixed == NODE_T_TOP */
    ...
    } else { /* nx == NODE_T_TOP && ny == NODE_T_TOP */
...
ushape_meet_pcons_ptr_neq(ushape_internal_t* pr,
    ...
    *rsize = 0;
    ...
    if (nnx != NODE_T_TOP && nny != NODE_T_TOP) {
        ...
        else if (c->info.ptr.nextx + c->info.ptr.nexty == 1) { /* v1->next != v2 : create a new node s.t. v1
                   * -->new node --> v2 */
            node_t n1, n2;
            size_t v1, v2;
            if (c->info.ptr.nextx == 1) {
        ...
        if ((nx != NODE_T_TOP && nnx == NODE_T_TOP
            && nny != NODE_T_TOP) || (ny != NODE_T_TOP
            && nny == NODE_T_TOP
            && nnx != NODE_T_TOP)) { /* v1->next != v2, v1->next undefined, v2
                   * fixed */
        node_t n1, n2;
        size_t v1, v2;
        if (nnx == NODE_T_TOP) {
        ...
        *rsize += ushape_array_add(pr, true, r, *rsize, true, true, b); /* copy and destroy */
        ...
        hgraph_node_forall(a->h, i) {
            ...
            (ny == NODE_T_TOP && nnx != NODE_T_TOP)) { /* case v1 != v2[->next] for v1 not yet fixed
                   * and v2[->next] fixed */
        node_t n1, n2;
        size_t v1, v2;
        if (nx == NODE_T_TOP) {
    ...
    } else { /* TODO: the same cases like for equality,
