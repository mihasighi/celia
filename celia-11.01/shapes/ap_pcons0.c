...
void
shape_offset_fprint(FILE * stream, int ofs, size_t dim) {
    switch (ofs) {
        case OFFSET_NEXT: fprintf(stream, " \\next(x%zu) ", dim);
            break;
        case OFFSET_DATA: fprintf(stream, " \\data(x%zu) ", dim);
            break;
        case OFFSET_LEN: fprintf(stream, " \\length(x%zu) ", dim);
            break;
        case OFFSET_SUM: fprintf(stream, " \\sum(x%zu) ", dim);
            break;
        case OFFSET_MSET: fprintf(stream, " \\mset(x%zu) ", dim);
            break;
        default:
            if (ofs >= 0)
...
void
shape_offsets_fprint(FILE * stream, int* offsets, size_t realdim) {
#ifndef NDEBUG
    ...
    if (offsets == NULL)
        ...
        for (i = 0; i < realdim; i++)
            shape_offset_fprint(stream, offsets[i], i);
        fprintf(stream, "]\n");
    }
}
...
shape_pcons_fdump(FILE * stream, pcons0_t * c) {
    if (!c)
        ...
        if (c->type == DATA_CONS) {
            ...
            shape_offsets_fprint(stream, c->info.data.offsets, c->ptrdim);
        } else {
            size_t i;
