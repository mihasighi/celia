...
mset_add_dimensions(ap_manager_t * man,
    ...
    r->dcons =
            ...
            project);
#else
    dimadd.intdim = addi + addr; // TODO: pb since some addi can be length variables
    dimadd.dim = (ap_dim_t *) malloc(dimadd.intdim * sizeof (ap_dim_t));
    size_t i;
    for (i = 0; i < addi; i++)
        dimadd.dim[i] = dimchange->dim[i];
...
mset_remove_dimensions(ap_manager_t * man,
    ...
    free(dimrm.dim);
#else
    dimrm.intdim = dimchange->intdim + dimchange->realdim;
    dimrm.dim = (ap_dim_t *) malloc(dimrm.intdim * sizeof (ap_dim_t));
    for (i = 0; i < dimchange->intdim; i++)
        dimrm.dim[i] = dimchange->dim[i];
    for (i = 0; i < dimchange->realdim; i++)
        dimrm.dim[dimchange->intdim + i] = -LEN_DIM(a->datadim, 0) + dimchange->dim[i];
    shape_dimchange_sort(&dimrm);
    r->dcons =
            ap_abstract0_remove_dimensions(pr->man_dcons, false, a->dcons, &dimrm);
    free(dimrm.dim);
    ...
    dimrm.dim = (ap_dim_t *) malloc(dimrm.intdim * sizeof (ap_dim_t));
    for (i = 0; i < dimchange->realdim; i++)
        dimrm.dim[dimchange->intdim + i] = -DATA_DIM(a->datadim, 0) + dimchange->dim[i];
    shape_dimchange_sort(&dimrm);
    r->lcons =
    ...
    for (i = 0; i < dimchange->intdim; i++)
        dimrm.dim[i] = dimchange->dim[i];
    for (i = 0; i < dimchange->realdim; i++) {
        dimrm.dim[dimchange->intdim + 2 * i + 0] = -LEN_DIM(a->datadim, 0) + dimchange->dim[i];
...
mset_split(mset_internal_t * pr, bool destructive, mset_t * a, ap_dim_t dim) {
...
#ifndef NDEBUG
    fprintf(stdout,
            "\n====mset_split: after substitute mstl(n1), before assign mstl(n1)\n");
    mset_fdump(stdout, pr->man, r);
    fflush(stdout);
#endif
    // mstl(n1) := 0
    ...
    r->mscons =
            ...
            NULL);
#ifndef NDEBUG
    fprintf(stdout, "\n====mset_split: after assign mstl(n1)\n");
    mset_fdump(stdout, pr->man, r);
    fflush(stdout);
#endif
...
#ifndef NDEBUG
    fprintf(stdout, "\n====mset_split - before substitute l[n1]\n");
    mset_fdump(stdout, pr->man, r);
    fflush(stdout);
#endif
    // l[n1] == l[n2] + 1;
