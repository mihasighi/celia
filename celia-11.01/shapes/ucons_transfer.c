...
size_t ** eq_mset_nodes = NULL;/*  array of mset inclusion between mset_nodes and
								 the other nodes
								 eq_mset_nodes[i][j] == true <=> mset(i) included in mset(j)
							  */
...
ucons_meet_lincons (ucons_internal_t * pr, bool destructive,
	...
	ap_dim_t n1, n2, n3; // inncoding to know when is
...
#ifndef NDEBUG1
		fprintf(stdout,"====ucons_meet_expr_constraint \n ");
...
#ifndef NDEBUG1
						fprintf(stdout,"\n ucons_strenghten debug insede meet a->udcons: \n");
						fprintf(stdout," step 3 res: \n");
...
void close_eq(bool ** tdim, size_t size){
	size_t i,j,k;
	size_t n = size;
	for(i=1;i<n; i++)
		for(j=0;j<=i; j++){
