...
ucons_t* add_pattern_n2p(ucons_internal_t *pr, ucons_t *r, pattern_key_t * key){
	...
	for(i=0 ; i<(u_seg+e_seg); i++){
		...
		checked_malloc(r->n2p[dim].p[size], pattern_key_t,1,sizeof(pattern_key_t)+(u_seg+e_seg)*sizeof(size_t),return NULL;);
	...
	for(s=a->udcons; s!=NULL; s=s->hh.next){
	  ...
	  if it doesn't exist remove otherwise modify
		...
		if(new_segments_size==(u_seg+e_seg)){
			...
			if(r_pattern){
				r_pattern->dcons =  ap_abstract0_remove_dimensions(pr->man_dcons,destructive,s->dcons,&dimrm);
			}
			else{
...
ucons_split (ucons_internal_t * pr, bool destructive, ucons_t * a, ap_dim_t dim)
{
	...
	/*
	 * meet with l[n1] > 1 on existential plus universals (this is the precondition under which split is called
	...
	for(j = 0; j < n1_pat_set.size; j++){
		...
		if(n1_pat_set.p[j]!=NULL){
			...
			if(aux){
				 ...
				 */
				pattern_sat = test_pattern_sat(pr,r,pattern_j,0);
...
#ifndef NDEBUG1
	fprintf(stdout,"\t split patterns of : \n");
	ucons_fprint(stdout,pr->man,a, NULL);
	fprintf(stdout,"\n resulting: \n");
	ucons_fprint(stdout,pr->man,r, NULL);
...
#ifndef NDEBUG1
	fprintf(stdout,"\t split before update lenghts : \n");
	ucons_fprint(stdout,pr->man,r, NULL);
	fprintf(stdout,"\n");
	fflush(stdout);
#endif
	...
	if(pr->active_patterns[2]){
		...
		if(first<pr->segm_anon){
			...
			if( first == 1 ){
...
#ifndef NDEBUG1
					fprintf(stdout,"\t active1= %zu size_fold_dim=%zu \t fold_dim: \n",pr->active_patterns[1],size_fold_dim);
					for(size_t o = 0;o<size_fold_dim;o++)
						fprintf(stdout,"\t %zu ",fold_dim[o]);
					fflush(stdout);
#endif
					/* segmanon = 1 therefore fold with closure and with the other patterns */
					r = fold_with_closoure_of_P21(pr,a,fold_dim, size_fold_dim,0,false);
					ucons_t* rr = fold_with_closure_P11_or_P12(pr,r,fold_dim,size_fold_dim,true);
					ucons_free_internal(pr,r);
					r = NULL;
					r = rr;
				}
...
#ifndef NDEBUG1
					fprintf(stdout,"\t not active1= %zu size_fold_dim=%zu \t fold_dim: \n",pr->active_patterns[1],size_fold_dim);
					for(size_t o = 0;o<size_fold_dim;o++)
						fprintf(stdout,"\t %zu ",fold_dim[o]);
					fflush(stdout);
#endif
					//no other pattern just fold with closure of P21
					r = fold_with_closoure_of_P21(pr,a,fold_dim, size_fold_dim,0,true);
				}
			}
			else{
				...
				r = fold_without_closoure_of_P21(pr,a,fold_dim, size_fold_dim);
			}
		...
		if(pr->active_patterns[1]){
			r = fold_with_closure_P11_or_P12(pr,a,tdim,size,true);
		}
	}
	//#else
...
ucons_t * merge_succesors_2(ucons_internal_t *pr, ucons_t * r, ap_dim_t * gdim,
...
ucons_t * merge_succesors_1(ucons_internal_t *pr, ucons_t * r, ap_dim_t * gdim,
		size_t size_gdim,bool update_lenght){
	...
	for(i = 1; i < size_cdim; i++){
		...
		if(n_dcons){
			...
			if(n_dcons->dcons){
				...
				else { aux_dcons = ap_abstract0_copy(pr->man_dcons, aux_ndcons);
...
ap_abstract0_t *concat_P12 (ucons_internal_t * pr, ucons_t * r, ap_dim_t * cdim,
	...
	for(i = 1;i < size_cdim; i++){
		...
		if(!p11_dcons){
			...
			dimchange.dim = (ap_dim_t*) malloc (2*sizeof(ap_dim_t));
			dimchange.dim[0] = r->datadim + 2*r->segmentdim;
			dimchange.dim[1] = r->datadim + 2*r->segmentdim+1; // adaugat +1

			p11_datacons = ap_abstract0_copy(pr->man_dcons, p11_dcons->dcons);
			p11_datacons = ap_abstract0_add_dimensions(pr->man_dcons, true, p11_datacons, &dimchange, false);
			free(dimchange.dim);
			/*
			 * added
			 */
		...
		ap_abstract0_t *aux_i_ydconsi = NULL;
		if(p11_datacons){
			ap_lincons0_array_t arr = ap_lincons0_array_make (2);
			...
			aux_i_ydconsi = ap_abstract0_meet_lincons_array (pr->man_dcons,
					false, p11_datacons, &arr);
...
void update_lenghts(ucons_internal_t *pr, ucons_t *r, ap_dim_t* cdim, size_t size_cdim ){
	ap_dim_t l0,li;
	size_t i;
	/* update length for the merged segments */
	/* l(cdim[0]) = l(cdim[0]) + l(cdim[1]) + ... + l(cdim[last])*/
