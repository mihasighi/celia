...
//ucons_t *  generate_pattern_2_1(ucons_internal_t * pr, ucons_t *r, ucons_t * a, ap_dim_t * tdim,
//		size_t size_tdim, pattern_key_t  * pattern_j){
//	return r;
//}
//ucons_t *  generate_pattern_2_1_len(ucons_internal_t * pr, ucons_t *r, ucons_t * a, ap_dim_t * tdim,
//		size_t size_tdim, pattern_key_t  * pattern_j, size_t pos_hd){
//	return r;
//}
//
//ucons_t* concat_nodes_P21(ucons_internal_t *pr, ucons_t * r, ap_dim_t* cdim, size_t size_cdim){
//	/* recalculate length of tdim[0] */
//#ifndef NDEBUG1
//	printf("\n concat_nodes_21 returs \n");
	...
	while(i<size_node_tdim){
		...
		if(ap_abstract0_sat_lincons(pr->man_dcons,r->econs,&cons)){
			...
			if (i-start==0){
				...
				if(pr->active_patterns[1] && !pr->active_patterns[4]){
...
#ifndef NDEBUG1
				fprintf(stdout,"\n\t gdim =  ");
				for(size_t o = 0;o<size_gdim;o++)
					fprintf(stdout,"\t %zu ",gdim[o]);
				fflush(stdout);
#endif
				r=merge_succesors_1(pr, r, gdim, size_gdim,update_lenght);
			}
...
#ifndef NDEBUG1
		fprintf(stdout,"\n\t cdim =  ");
		for(size_t o = 0;o<size_cdim;o++)
			fprintf(stdout,"\t %zu ",cdim[o]);
		fflush(stdout);
#endif
...
ucons_t *  ucons_fold_pattern_2_1(ucons_internal_t * pr, ucons_t *r, ucons_t * a, ap_dim_t * tdim,
	...
	for(ctr = 1; ctr<size_tdim; ctr++){
		...
		checked_realloc(len_e_seg, ap_dim_t, (le_seg+1), sizeof(ap_dim_t), return NULL;);
		size_t ii = le_seg;
	...
	for(i=0; i<size_node_tdim; i++){
		...
		if(update_length == true || (!update_length&&pr->active_patterns[1])){ // PB
			...
			if(node_tdim[i]>=a->datadim){
				size_tdim++;
				checked_realloc(tdim,ap_dim_t,size_node_tdim,sizeof(ap_dim_t),return NULL;);
...
ucons_t * fold_without_closoure_of_P21(ucons_internal_t * pr, ucons_t * a, ap_dim_t *fold_dim,
	...
	ap_dim_t *tdim = NULL;
	checked_realloc(tdim, ap_dim_t, size, sizeof(ap_dim_t), return NULL;);

	for(i = 0; i < size; i++){
		tdim[i] = fold_dim[i+size];
	}
	checked_realloc(fold_dim, ap_dim_t,
			size_fold_dim, sizeof(ap_dim_t), return NULL;);
...
//
//			if (test_singleton(pr->man_dcons, r->econs, r->datadim, r->segmentdim, fst_node_tdim[0])){
//				n_dcons->dcons = ap_abstract0_top(pr->man_dcons, r->datadim + 2*r->segmentdim + 2*nr_y, 0);
...
bool test_l_leq_2(ap_manager_t * man, ap_abstract0_t * dcons, size_t intdim, size_t realdim, ap_dim_t n){
	...
	linexpr = ap_linexpr0_alloc ( AP_LINEXPR_DENSE, intdim + 2 * realdim);
	ap_linexpr0_set_coeff_scalar_int (linexpr,intdim + realdim + n , 1);
	ap_linexpr0_set_cst_scalar_int (linexpr, -2);
