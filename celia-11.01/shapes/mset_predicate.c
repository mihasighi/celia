...
mset_is_dimension_unconstrained(ap_manager_t * man, mset_t * a, ap_dim_t dim) {
    ...
    if (mset_is_top(man, a))
        return true;
    bool is_uncons = true;
    if (dim < LEN_DIM(a->datadim, 0)) { // length program variables are constrained by lcons or dcons
#ifdef MSET_REL
        is_uncons = is_uncons
...
#endif
    } else if (dim < a->datadim) { // data program variables are constrained by dcons and mscons
#ifdef MSET_REL
        is_uncons = is_uncons
                && ap_abstract0_is_dimension_unconstrained(pr->man_dcons, a->dcons, dim);
#else
