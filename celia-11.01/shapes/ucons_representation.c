...
void initialize_PI( ucons_internal_t *pr){
	...
	checked_malloc(PI, pattern_info_t , 16,sizeof(pattern_info_t),return ; );

	PI_size = 16;
	/* \forall y. */
	pattern_info_t p1,p2,p3,p4,p5,p6,p7,p8,p9,p10,p11,p12,p13,p14,p15,p16;
	...
	/* \forall y1\in ? y2\in ?? . y1 + l[!!!]+ l[!!] + l[!]  =y2*/
	p16.kind=pattern_2_1_mlx;
	p16.e_seg = 3;
	p16.u_seg = 2;
	//p1.uvar=NULL;
	checked_malloc(p16.uvar,pattern_var_t,2,sizeof(pattern_var_t),return ;);
	p16.uvar[0].size=1;
	p16.uvar[0].reach=NULL;
	p16.uvar[1].size=1;
	p16.uvar[1].reach=NULL;
	p16.lcons= 1;// the second constaint y1=l[x]+y2
	p16.nr_y = 2;

	PI[0] = p1; PI[1] = p2; PI[2] = p3; PI[3] = p4;
	PI[4] = p5; PI[5] = p6; PI[6] = p7; PI[7] = p8;
	PI[8] = p9; PI[9] = p10; PI[10] = p11; PI[11] = p12;
	PI[12] = p13; PI[13] = p14; PI[14] = p15; PI[15] = p16;
...
ucons_approximate (ap_manager_t * man, ucons_t * a, int algorithm)
{
	...
	/*
	 * patts incodes a number on 3 bits
	 * set active patterns
	 * 1 = 0 0 1 -> P11
	 * 2 = 0 1 0 -> P21
	 * 5 = 1 0 1 -> P12 //always comes with closure
	 */
	size_t ind,k;
	...
	while(patt>0){
		ind = patt%2;
		if (ind!=0) pr->active_patterns[ind*k] = true;
		k=k*2;
		patt = patt/2;
	}
...
#ifndef NDEBUG1
	fprintf(stdout, "active patterns in aproximate ");
	size_t i;
...
#ifndef NDEBUG1
	fprintf(stdout, "ucons_approximate: a!=NULL pattern removed  %zu \n ", types);
	fflush(stdout);
#endif
...
//		pr->man_dcons = ap_ppl_poly_manager_alloc(false);
//		//pr->man_dcons = pk_manager_alloc (false);
//		//ap_ppl_poly_manager_alloc(false);
//		break;
//	case DOM_POLY_P21:
//		fprintf(stdout,"alege ppl");
//		fflush(stdout);
