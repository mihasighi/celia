...
/**
 * Substitute the variables in tdim by the corresponding variables
 * in tnew. 
 * @param size length of tnew and tdim
 * @return 
 */
shape_t*
shape_substitute_array(shape_internal_t* pr,
        bool destructive,
        shape_t* a,
        ap_dim_t* tdim,
        ap_dim_t* tnew,
    ...
    for (i = 0; i < a->msize; i++) {
        ushape_array_t *rr =
                ushape_substitute_array(pr, false, a->m.p[i], tdim, tnew, size);
        if (rr) {
            r = ushape_array_add_array(pr, true, r, rr);
            ushape_array_init(pr, rr, rr->size);
            free(rr);
...
/** Used by the implementation of return edge in ICFG.
...
#endif
        /* go */
        r = shape_substitute_array(pr, false, a, tdim, tnew, size);
        if (dest) { /* intersect r with dest */
            shape_t *rr = shape_meet(pr->man, false, r, dest);
            shape_free_internal(pr, r);
            return rr;
