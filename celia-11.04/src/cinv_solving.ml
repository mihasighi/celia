  ...
  let info = PSHGraph.info graph in
  {
    ...
    Fixpoint.bottom = begin fun vtx ->
	 ...
	 begin
	   ...
	   failwith "Vertex not found (finxpoint.bottom\n"
	 end
      )
    end;
...
(** We compute the relation corresponding to the input heap. **)
  ...
  let apply_call_main
    ...
    let env_oin =
      environment_of_tvar 
	(Apron.Environment.typ_of_var maininfo.Cinv_equation.penv) 
	maininfo.Cinv_equation.pinput_old in
    let env_in_oin = 
      Apron.Environment.lce env_oin env_in in
    let abstract2 =
      Apron.Abstract1.change_environment manager 
	abstract1 env_in_oin true (* DUP *) in 
  ...
  let compute
    ...
    let sstart =
	begin
	  if 0 <> (Apron.Tcons1.array_length maininfo.Cinv_equation.prequires) then
