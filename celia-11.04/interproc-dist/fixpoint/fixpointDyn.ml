...
let add_vertex
  ...
  let attr = { reach = bottom; empty = true; } in
...
let propagate
  ...
  if guided then begin
	...
	if FixpointStd.is_tvertex graph tpredvertex then begin
	  ...
	  let treach = FixpointStd.treach_of_tvertex graph tpredvertex in
...
let analysis
  ...
  if not (PSette.is_empty info.iinit) then begin
    Time.wrap_duration_add info.itime
      (fun () -> info.istable := not(fixpoint ~guided manager graph make_strategy))
  end;
  if manager.print_analysis then
    fprintf manager.print_fmt ".. in %i,%i iterations %s stabilization ***@."
      !(info.iiterations)
      !(info.idescendings)
      (if !(info.istable) then "with" else "without")
  ;
...
let graph_of_equation
  ...
  let rec explore vertex =
    if not (PSHGraph.is_vertex graph vertex) then begin
