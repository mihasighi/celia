...
let rec force_app_term_type f = function
  ...
  | Ltype _ | Lvar _ | Linteger | Lreal | Larrow _ as ty ->
      Celia_options.fatal "Unexpected non-C type %a" !Ast_printer.d_logic_type ty
...
let get_unique_field ty = match unrollType ty with
  | TComp(compinfo,_,_) ->
	...
	| _ ->
	    Celia_options.fatal "Type %a has no unique field" !Ast_printer.d_type ty
      end
...
object
  ...
  method vrooted_code_annotation = visitor#vrooted_code_annotation
  method vglob_aux g =
    Celia_options.feedback "%a" !Ast_printer.d_global g;
    visitor#vglob_aux g
  method vstmt_aux s =
    Celia_options.feedback "%a" !Ast_printer.d_stmt s;
    visitor#vstmt_aux s

  (* Methods from Cil visitor for coding constructs *)
  method vfunc = visitor#vfunc
  method vblock b =
    Celia_options.feedback "%a" !Ast_printer.d_block b;
    visitor#vblock b
  method vvrbl v =
  ...
  method vvdec v =
    ...
    visitor#vvdec v
  method vexpr e =
    Celia_options.feedback "%a" !Ast_printer.d_exp e;
    visitor#vexpr e
  method vlval lv =
    Celia_options.feedback "%a" !Ast_printer.d_lval lv;
    visitor#vlval lv
  method voffs off =
    Celia_options.feedback "%a" !Ast_printer.d_offset off;
    visitor#voffs off
  method vinitoffs off =
    Celia_options.feedback "%a" !Ast_printer.d_offset off;
    visitor#vinitoffs off
  method vinst i =
    Celia_options.feedback "%a" !Ast_printer.d_instr i;
    visitor#vinst i
  method vinit = visitor#vinit
  method vtype ty =
    Celia_options.feedback "%a" !Ast_printer.d_type ty;
    visitor#vtype ty
  method vattr attr =
    Celia_options.feedback "%a" !Ast_printer.d_attr attr;
    visitor#vattr attr
  method vattrparam pattr =
    Celia_options.feedback "%a" !Ast_printer.d_attrparam pattr;
    visitor#vattrparam pattr

  (* Methods from Cil visitor for logic constructs *)
  method vlogic_type lty =
    Celia_options.feedback "%a" !Ast_printer.d_logic_type lty;
    visitor#vlogic_type lty
  method vterm t =
    Celia_options.feedback "%a" !Ast_printer.d_term t;
    visitor#vterm t
  method vterm_node = visitor#vterm_node
  method vterm_lval tlv =
    Celia_options.feedback "%a" !Ast_printer.d_term_lval tlv;
  ...
  method vlogic_var_decl lv =
    Celia_options.feedback "%a" !Ast_printer.d_logic_var lv;
  ...
  method vpredicate_named p =
    Celia_options.feedback "%a" !Ast_printer.d_predicate_named p;
...
*)
  ...
  method vspec funspec =
    Celia_options.feedback "%a" !Ast_printer.d_funspec funspec;
  ...
  method vcode_annot annot =
    Celia_options.feedback "%a" !Ast_printer.d_code_annotation annot;
    visitor#vcode_annot annot
  method vannotation annot =
    Celia_options.feedback "%a" !Ast_printer.d_annotation annot;
  ...
  method vterm t =
    match t.term_node with
	  ...
	  begin match t.term_type with
		...
		Celia_options.fatal ~current:true "Term %a has void type" !Ast_printer.d_term t
	    | _ -> DoChildren
	  end
      | _ -> DoChildren
