...
/* Offsets shall be negative because dimensions are positive */
    typedef enum {
        ...
        OFFSET_UCONS = -6, /* for mset function */
        OFFSET_OTHER = -7, /* not to be used */
    } offset_t;

    typedef struct {

        enum {
            DATA_CONS = 0, /* only constraint on data vars or data
				 * fields */
...
/* Array of constraints */
    ...
    void shape_offset_fprint(FILE * stream, int ofs, size_t datadim, size_t dim);

    void shape_offsets_fprint(FILE * stream, int* offsets, size_t datadim, size_t dim);
