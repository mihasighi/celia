...
ucons_t *
ucons_unfold (ucons_internal_t * pr, bool destructive, ucons_t * a, ap_dim_t dim)
{
	...
	/*
	 * meet with l[n1] > 1 on existential plus universals (this is the precondition under which unfold is called
	...
	///////***********************
#ifndef NDEBUG2
	fprintf(stdout,"\t actually ++++ucons_unfold: \n");
	ucons_fprint(stdout,pr->man,r, NULL);
	fprintf(stdout,"\n");
#endif
	...
	for(j = 0; j < n1_pat_set.size; j++){
		...
		if(n1_pat_set.p[j]!=NULL){
			...
			if(aux){
				 ...
				 * because unfoldting DOES NOT consider sub-patterns
				 */
...
#ifndef NDEBUG1
	fprintf(stdout,"\t ++++ucons_unfold patterns of : \n");
	ucons_fprint(stdout,pr->man,a, NULL);
	fprintf(stdout,"\n resulting: \n");
	ucons_fprint(stdout,pr->man,r, NULL);
...
#ifndef NDEBUG1
	fprintf(stdout,"\t ++++ucons_unfold before update lenghts : \n");
	ucons_fprint(stdout,pr->man,r, NULL);
	fprintf(stdout,"\n");
	fflush(stdout);
#endif
...
#ifndef NDEBUG1
	fprintf(stdout,"\t ++++ucons_unfold returns : \n");
	ucons_fprint(stdout,pr->man,r, NULL);
	fprintf(stdout,"\n");
#endif
...
ucons_expand (ap_manager_t * man,
	...
	if (!a)
		return NULL;
	arg_assert (n <= 3 && dim > 0 && dim < a->segmentdim, return NULL;);
	ucons_t *r;
	switch(n) {
            case 1:
		r = ucons_singleton (pr, destructive, a, dim); break;
            case 2:
		r = ucons_unfold (pr, destructive, a, dim); break;
            default:
	...
	if(pr->active_patterns[2]){
		...
		if(first<pr->segm_anon){
			...
			if( first == 1 ){
				...
				r = fold_with_closoure_of_P21(pr,a,fold_dim, size_fold_dim,0,(up_len==1));
				up_len --;
...
ucons_t* concat_nodes_1(ucons_internal_t *pr, ucons_t * r, ap_dim_t* cdim,
	...
	if (!(cdim && size_cdim > 1)) {
#ifndef NDEBUG2
	printf("  \n\t concat_nodes_1 : failed assert (cdim && size_cdim > 1)\n");
#endif
	   return r;
        }
