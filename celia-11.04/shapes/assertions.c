...
build_const_5(ucons_internal_t * pr, ucons_t * r,
    ...
    arg_assert(smain == 2 && saux == 1, return r;);
...
#ifndef NDEBUG1
    fprintf(stdout, "\n@@@@ ucons_build \\forall y1,y2 in [n%zu]. y2=y1+1 ==> d(y2)-d(y1)>=y1-2\n",
            nmain[0] - r->datadim);
    fflush(stdout);
#endif
...
build_const_20(ucons_internal_t * pr, ucons_t * r,
    ...
    arra = ap_lincons0_array_make((saux == 0) ? 1 : 2);
    /*
     *  d(n) <= d(y) <= naux
     */
    arra.p[0].constyp = AP_CONS_SUPEQ;
    ...
    if (saux == 1) {
        ...
        arra.p[1].linexpr0 =
                ap_linexpr0_alloc(AP_LINEXPR_DENSE, r->datadim + 2 * r->segmentdim + 2);
        arra.p[1].scalar = NULL;
        ap_linexpr0_set_coeff_scalar_int(arra.p[1].linexpr0, dy1, -1);
        ap_linexpr0_set_coeff_scalar_int(arra.p[1].linexpr0, naux[0], 1);
        ap_linexpr0_set_cst_scalar_int(arra.p[1].linexpr0, 0);
    }
...
ucons_t* build_const_11(ucons_internal_t * pr, ucons_t * r,
    ...
    ap_linexpr0_set_cst_scalar_int(arr.p[1].linexpr0, 2);
