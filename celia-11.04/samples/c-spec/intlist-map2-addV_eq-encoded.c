...
/*********************************************************/
/* Program for the verification of intlist-map2-addV_eq.c */
/*********************************************************/
...
size_t intdim = 1; /* vars c */
char* names_dim[6] = {"c", "tmp", "x", "xi", "y", "yi"};
...
void addV_eq(void) {
    ...
    shape_t *top;
    shape_t *shp_pre; /* acyclic(x) && acyclic(y) && len(x)==len(y)>=1 && xi(#) && yi(#) && tmp(#) */
    shape_t *sid_3; /* xi = x */
    shape_t *sid_4; /* yi = y */
    shape_t *sid_5; /* while (xi != NULL) */
    shape_t *sid_6; /* yi->data = xi->data + c; */
    ...
    printf("\nvvvvvvvvvv addV_eq (ok expected) vvvvvvvvvv\n");
     ...
     *       forall y1, y2. 1<=y1<\len(n1) /\ 1<=y2<\len(n2) /\ y1=y2 ==> \data(y1)=\data(y2)+c
     */
    ...
    t6 = shape_texpr_deref_data(
            ...
            ap_texpr0_dim(DIM_C), AP_RTYPE_INT, AP_RDIR_RND)); /* xi->data+c / data */
    ...
    sid_6 = shape_assign_texpr_array(ms, true, sid_5, &yi, &t6, 1, NULL); /* yi->data = xi->data + c; */
